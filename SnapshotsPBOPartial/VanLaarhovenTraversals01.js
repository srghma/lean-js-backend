...
const traverseFun1 = (dictApplicative) => {
  ...
  return (k) => (v) => {
    if (v.tag === "Abs") {
      return $0.map(Abs(v._1))(k(v._2));
    }
    if (v.tag === "App") {
      return Apply0.apply($0.map(App)(k(v._1)))(k(v._2));
    }
...
const rewriteBottomUpM = (dictMonad) => {
  ...
  return (k) => {
    const go = (a) => {
      if (a.tag === "Abs") {
        return $0.map(Abs(a._1))($1.bind(go(a._2))(k));
      }
      if (a.tag === "App") {
        return Apply0.apply($0.map(App)($1.bind(go(a._1))(k)))(
          $1.bind(go(a._2))(k),
        );
      }
      throw new Error("UNREACHABLE");
...
const rewriteBottomUp = (k) => {
  const go = (a) => {
    if (a.tag === "Abs") {
      return $Fun("Abs", a._1, k(go(a._2)));
    }
    if (a.tag === "App") {
      return $Fun("App", k(go(a._1)), k(go(a._2)));
    }
    throw new Error("UNREACHABLE");
  };
  return (a) => k(go(a));

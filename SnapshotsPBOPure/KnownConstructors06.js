...
const genericTest = {
  ...
  from: (x) => {
    if (x === "Foo") {
      return Data$dGeneric$dRep.$Sum("Inl", Data$dGeneric$dRep.NoArguments);
    }
    if (x === "Bar") {
      return Data$dGeneric$dRep.$Sum(
        "Inr",
        Data$dGeneric$dRep.$Sum("Inl", Data$dGeneric$dRep.NoArguments),
      );
    }
    if (x === "Baz") {
      return Data$dGeneric$dRep.$Sum(
    ...
    if (x === "Qux") {
      return Data$dGeneric$dRep.$Sum(
        ...
        Data$dGeneric$dRep.$Sum(
          "Inr",
          Data$dGeneric$dRep.$Sum("Inr", Data$dGeneric$dRep.NoArguments),
        ),
      );
    }
    throw new Error("UNREACHABLE");

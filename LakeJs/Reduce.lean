...
at all: `Tail.lsubst` binds each non-atomic argument with a `let` in front of the block
...
Two examples of the rule that keeps a control transfer call-by-value: an **atomic**
argument — a variable or a literal — is substituted into the block, and anything else is
bound by a `let` in front of it, so it is run once, before the block. -/

/-- `Term.sharedTail` jumps with literals, so inlining its label substitutes them and the
    answer is the branch, with no binding made. -/
example :
    Step (Sg := sigNone) (Γ := [Ty.bool]) Term.sharedTail
      (.block (.iteT (♯0) (.ret (.lit (.nat 1))) (.ret (.lit (.nat 2))))) :=
  .blockStep .labelJoin

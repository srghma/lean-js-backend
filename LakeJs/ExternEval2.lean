...
def LeanInitPureExtern2OnlyPrim.eval : ∀ {a b c : LeanPrimTy},
  ...
  | _, _, _, .lean_substring_prev, v, w => ((Substring.Raw.prev v ⟨w⟩).byteIdx)
  ...
  | _, _, _, .lean_string_utf8_next, v, w => ((String.Pos.Raw.next v ⟨w⟩).byteIdx)
  | _, _, _, .lean_string_pos_min, v, w => (min v w)
  | _, _, _, .lean_string_pos_sub, v, w => (v - w)
  ...
  | _, _, _, .lean_string_utf8_next_basic, v, w => ((String.Pos.Raw.next v ⟨w⟩).byteIdx)
  | _, _, _, .lean_string_pos_raw_next, v, w => ((String.Pos.Raw.next v ⟨w⟩).byteIdx)
  | _, _, _, .lean_string_pos_raw_get, v, w => (String.Pos.Raw.get v ⟨w⟩)
  | _, _, _, .lean_string_get_basic, v, w => (String.Pos.Raw.get v ⟨w⟩)
  | _, _, _, .lean_string_pos_raw_prev, v, w => ((String.Pos.Raw.prev v ⟨w⟩).byteIdx)
  | _, _, _, .lean_string_prev, v, w => ((String.Pos.Raw.prev v ⟨w⟩).byteIdx)
  | _, _, _, .lean_string_next_fast, v, w => ((String.Pos.Raw.next v ⟨w⟩).byteIdx)
  | _, _, _, .lean_string_pos_raw_next_fast, v, w => ((String.Pos.Raw.next v ⟨w⟩).byteIdx)
  | _, _, _, .lean_string_pos_next, v, w => ((String.Pos.Raw.next v ⟨w⟩).byteIdx)
  ...
  | _, _, _, .lean_string_decode_char, v, w => (String.Pos.Raw.get v ⟨w⟩)
  ...
  | _, _, _, .lean_string_posof, v, w => (strPosOf v w)
  ...
  | _, _, _, .lean_string_uget_byte_fast, v, w => ((String.toUTF8 v)[w.toNat]!)
  | _, _, _, .lean_slice_dec_lt, v, w => (decide (v.toString < w.toString))

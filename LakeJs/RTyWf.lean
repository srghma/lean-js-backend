...
`LakeJs.Schema`, so they need no check at all.  What is left are the conditions that
...
stated here, *before* `LakeJs.Ty`, for one reason: the recursive constructors of `Ty`
...
exists.  They are what the recursive constructors of `LakeJs.Ty` ask a proof of. -/
...
/-- `Ty.wf`, one layer down: this type may mention the declaration it sits in.
    ...
    `LakeJs/DivergeNeg.lean` used to prove.  Nothing a Lean source declaration needs is
...
them, so there is no `Ty` for them: the corresponding constructor of `LakeJs.Ty` cannot
be applied. -/

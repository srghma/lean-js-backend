...
Binding *every* argument, rather than copying the ones that are a variable or a literal,
is what makes label substitution commute with value substitution — a variable argument
becomes an arbitrary value once the block is closed, and a copying rule would then have
to bind it after all.  That commutation is what the join-point certificate generator of
`LakeJs.CertGen` rests on; the extra `let` it costs binds an argument that is already a
value, and is removed by any copy-propagation pass on the emitted code.
-/
...
Copying an argument duplicates a computation, and copying it *unevaluated* is not
call-by-value either.  `Tail.letSpine` is what a jump does with its arguments instead of
substituting them: each of them is bound by a `let`, which the evaluator runs before it
runs the block. -/

...
public import LakeJs.LeanInitPureExterns
...
`LakeJs.LeanInitPureExterns` lists the pure `@[extern]` functions of Lean's `Init`,
...
`LakeJs.ExternEval`, `LakeJs.ExternEval1`, `LakeJs.ExternEval2` and
`LakeJs.ExternEvalMisc`, and `LakeJs.Expr.Step` runs it (`EXTERN_EVALUATION.md`).
-/
...
/-- A function of the runtime, with the types of its arguments and of its result.  The
    constructors are the families of `LakeJs.LeanInitPureExterns`, instantiated here. -/

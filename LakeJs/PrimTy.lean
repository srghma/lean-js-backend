...
/-!
# `PrimTy`: the terminal (leaf) types
...
* a backend that only has to decide "how is a scalar represented?" pattern-matches on
  `PrimTy`, a small type with no recursion and no indexed families, and can `deriving
...
* the "how is this rendered in JS?" configuration (`number` vs `bigint`, …) is a
  function `PrimTy → …` — see `LakeJs.Config` — rather than a function on `Ty` with
  unreachable cases.
...
information and is erased before it reaches `PrimTy`, and a type with no values has no
runtime representation at all.  This is also why `bitvec n` requires `0 < n`:
`BitVec 0` is a unit type.
-/

/-- A terminal type: a leaf of a `Ty`, with a built-in JavaScript representation. -/
inductive PrimTy where
  ...
  /-- IF `n` is less THEN 32 then nonconfigurable (`number`), ELSE configurable
      ...
      works. -/
  | bitvec    : (n : Nat) → (h_pos : 0 < n := by decide) → PrimTy
  /-- In JS: `number`. -/
  | uint8     : PrimTy
  /-- In JS: `number`. -/
  | uint16    : PrimTy
  ...
  | childProcess : PrimTy
  /-- In JS: `object` / `any`. -/
  | shareCommonObject : PrimTy
  /-- In JS: a `Map` / cache object. -/
  | shareCommonState  : PrimTy
  /-- Maps to `Nat` in Lean; in JS it is erased just like a unit value. -/
  | taskPriority : PrimTy
...
def isNumberConfigurable : PrimTy → Bool

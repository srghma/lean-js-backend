...
schema serves `LakeJs.Ty` (the JavaScript backend's type language), the layer inside a
recursive declaration (`LakeJs.Ty.RTy`), and any further language a later backend adds:

...
`RTy` is everything a closed type (`LakeJs.Ty`) can be, and in addition an occurrence of
the declaration being defined (`RTy.self i`).  It is defined **before** `Ty` and does not
mention it, which is what makes the design of `LakeJs.Ty` possible: the conditions that
...
(`LakeJs.RTyWf`), so the recursive constructors of `Ty` can *take the proof as an

...
* `toTy_class` — whenever the translation succeeds, the `Ty` constructor it produces
  is the one `classify` predicts, and `classify` is a function of the four
  observables of `ShapeDescriptor` alone.  Since `toTy` is a function, a declaration
  has exactly one translation; and since the six shapes are pairwise distinguished by
...
inductive Reject where
  ...
  | duplicateNames
  /-- A recursive declaration with no way to build a value: no base constructor, or an
      unguarded self occurrence in a record. -/
  | notWellFounded
...
/-- A declaration that is not part of a genuine mutual block: one of the five
    non-mutual shapes, chosen by "recursive?", "how many constructors?" and "any
    fields?". -/
def toSingleTy (self : Nat) (m : SrcMember) : Except Reject Ty :=
  if mentionsForeign m self then .error .foreignReference
  else if memberMentions m self then
    match m.ctors with
    | [] => .error .voidMember
    | [c] =>
        match c.fields with
        ...
        | f :: fs =>
            match buildSelfFields? self (f :: fs) with
            | none => .error .foreignReference
            | some row =>
                if h : recObjectShapeOk (selfFieldShape (f :: fs)) = true then
    ...
    | [c] =>
        ...
        | [] => .error .unitLikeMember
        | f :: fs =>
            match buildFields? (f :: fs) with
            | none => .error .foreignReference
            | some row =>
                if h : fieldShapeOk (plainFieldShape (f :: fs)) = true then
                  .ok (.record { name := m.name, fields := row, h_names := h })
                else .error .duplicateNames
    | c₁ :: c₂ :: cs =>
        if memberHasFields m then
...
/-- **The** translation of a source declaration into `Ty`.  It is a function, so a
    declaration has at most one translation; `toTy_class` shows the shape it picks is
    forced. -/
...
def singleSupported (self : Nat) (m : SrcMember) : Bool :=
  !mentionsForeign m self &&
    (match m.ctors with
     ...
     | [c] =>
         !c.fields.isEmpty &&
           (if memberMentions m self then recObjectShapeOk (selfFieldShape c.fields)
            else fieldShapeOk (plainFieldShape c.fields))
     | c₁ :: c₂ :: cs =>
...
/-- Which of the six shapes a `Ty` is; `none` for the built-in types, which are not
    user declarations. -/
  ...
  · simp only [Bool.not_eq_true] at hF
    ...
    · simp only [hR, if_true]
      ...
      · rcases cs with _ | ⟨c₂, cs⟩
        · have hcf : c.fields.all (fun f => refsOnly self f.2) = true :=
          ...
          · simp [hf]
          · have hs : (buildSelfFields? self (f :: fs)).isSome = true := by
              rw [buildSelfFields?_isSome]; rw [hf] at hcf; exact hcf
            rcases hb : buildSelfFields? self (f :: fs) with _ | row
            · rw [hb] at hs; simp at hs
            · by_cases hok : recObjectShapeOk (selfFieldShape (f :: fs)) = true <;>
                simp [hf, hb, hok]
        · have hcs : ((c :: c₂ :: cs).all
    ...
    · simp only [Bool.not_eq_true] at hR
      ...
      have hempty : ∀ c ∈ m.ctors,
        ...
        simp only [List.all_eq_true]
        intro f hf
        exact targets_isEmpty_of self f.2 (refsOnly_of_not_mentionsForeign hF hc hf)
          (not_contains_of_not_memberMentions hR hc hf)
      rcases hc : m.ctors with _ | ⟨c, cs⟩
      · simp
      · rcases cs with _ | ⟨c₂, cs⟩
        · have hcf : c.fields.all (fun f => f.2.kind.targets.isEmpty) = true :=
            hempty c (by rw [hc]; simp)
          rcases hf : c.fields with _ | ⟨f, fs⟩
          · simp [hf]
          · have hs : (buildFields? (f :: fs)).isSome = true := by
              rw [buildFields?_isSome]; rw [hf] at hcf; exact hcf
            rcases hb : buildFields? (f :: fs) with _ | row
            · rw [hb] at hs; simp at hs
            · by_cases hok : fieldShapeOk (plainFieldShape (f :: fs)) = true <;>
                simp [hf, hb, hok]
        · by_cases hfields : memberHasFields m = true
          · simp only [hfields, if_true]
            have hcs : ((c :: c₂ :: cs).all
...
theorem toTy_isOk_iff_supported (d : SrcDecl) : isOk d.toTy = supported d := by
...
/-- The non-mutual half of the uniqueness theorem. -/
theorem toSingleTy_class {self : Nat} {m : SrcMember} {t : Ty}
    (h : toSingleTy self m = .ok t) : tyClass t = singleClass self m := by
  ...
  all_goals
    first
      | (injection h with h'
         subst h'
         simp_all [tyClass, singleClass, ShapeDescriptor.class?, memberHasFields])
      | simp at h
...
theorem toTy_class {d : SrcDecl} {t : Ty} (h : d.toTy = .ok t) : tyClass t = classify d := by
  ...
  · simp only [hm] at h
    by_cases hmut : isGenuinelyMutual d.block = true
    · rw [if_pos hmut] at h
      rw [classify_eq_mutual d m hm hmut]
      exact toFamilyTy_class h
    · simp only [Bool.not_eq_true] at hmut
      rw [if_neg (by simp [hmut])] at h
      rw [classify_eq_singleClass d m hm hmut]
      exact toSingleTy_class h

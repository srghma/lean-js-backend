...
δ-rule of `LakeJs.Reduce` that runs a one-argument extern carries no side condition.
The entries that denoted no function of their argument's value are commented out of
`LakeJs.LeanInitPureExterns`, each with its reason; see `EXTERN_EVALUATION.md`.
-/
...
def LeanInitPureExtern1OnlyPrim.eval : ∀ {a b : LeanPrimTy},
  ...
  | _, _, .lean_int_dec_nonneg, v => (decide (0 ≤ v))
  | _, _, .lean_int_neg_succ_of_nat, v => (Int.negSucc v)
  | _, _, .lean_int_neg, v => (-v)
  ...
  | _, _, .lean_string_trim, v => ((String.trimAscii v).toString)
  | _, _, .lean_substring_tostring, v => (Substring.Raw.toString v)
  | _, _, .lean_string_isempty, v => (String.isEmpty v)
  | _, _, .lean_string_front, v => (String.front v)
  | _, _, .lean_string_length, v => (String.length v)
  | _, _, .lean_string_capitalize, v => (String.capitalize v)
  | _, _, .lean_substring_front, v => (Substring.Raw.front v)
  | _, _, .lean_substring_isempty, v => (Substring.Raw.isEmpty v)
  | _, _, .lean_string_of_usize, v => (toString v)
  | _, _, .lean_nat_log2, v => (Nat.log2 v)
  | _, _, .lean_uint16_neg, v => (-v)
  | _, _, .lean_uint16_to_usize, v => (UInt16.toUInt64 v)
  | _, _, .lean_uint64_complement, v => (~~~v)
  | _, _, .lean_bool_to_uint32, v => ((if v then 1 else 0))
  | _, _, .lean_uint8_neg, v => (-v)
  | _, _, .lean_uint16_complement, v => (~~~v)
  | _, _, .lean_uint32_neg, v => (-v)
  | _, _, .lean_usize_neg, v => (-v)
  | _, _, .lean_usize_to_uint64, v => (v)
  | _, _, .lean_bool_to_uint64, v => ((if v then 1 else 0))
  | _, _, .lean_usize_of_nat32, v => (UInt64.ofNat v)
  | _, _, .lean_uint64_neg, v => (-v)
  | _, _, .lean_uint32_to_usize, v => (UInt32.toUInt64 v)
  | _, _, .lean_uint8_complement, v => (~~~v)
  | _, _, .lean_usize_to_uint16, v => (UInt64.toUInt16 v)
  | _, _, .lean_usize_to_uint8, v => (UInt64.toUInt8 v)
  | _, _, .lean_bool_to_uint8, v => ((if v then 1 else 0))
  | _, _, .lean_uint32_complement, v => (~~~v)
  | _, _, .lean_uint8_to_usize, v => (UInt8.toUInt64 v)
  | _, _, .lean_bool_to_uint16, v => ((if v then 1 else 0))
  | _, _, .lean_bool_to_usize, v => ((if v then 1 else 0))
  | _, _, .lean_uint64_to_usize, v => (v)
  | _, _, .lean_usize_to_uint32, v => (UInt64.toUInt32 v)
  | _, _, .lean_usize_complement, v => (~~~v)
  | _, _, .lean_string_length_def, v => (String.length v)
  | _, _, .lean_isize_complement, v => (~~~v)
  ...
  | _, _, .lean_int32_neg, v => (-v)
  | _, _, .lean_int32_abs, v => (Int32.abs v)
  | _, _, .lean_bool_to_int8, v => ((if v then 1 else 0))
  | _, _, .lean_isize_to_int16, v => (Int64.toInt16 v)
  | _, _, .lean_int16_to_int, v => (Int16.toInt v)
  | _, _, .lean_int8_complement, v => (~~~v)
  | _, _, .lean_bool_to_int16, v => ((if v then 1 else 0))
  ...
  | _, _, .lean_float_to_string, v => (toString v)
  ...
  | _, _, .lean_float32_to_string, v => (toString v)

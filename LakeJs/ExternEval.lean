...
The evaluator of `LakeJs.Reduce` must be able to *run* `lean_nat_add 1 2`, not merely to
stop in front of it, so the meaning of the catalogue of `LakeJs.Externs` is given here:
a Lean function for each entry, on the Lean values the literals of `LakeJs.LeanPrimTyLit`
hold.
...
`LakeJs.LeanInitPureExterns` — not deleted — with the reason attached to each line:
